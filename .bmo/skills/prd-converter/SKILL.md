...
```json
{
  ...
  "branchName": "bmo/[feature-name-kebab-case]",
...
**Each story must be completable in ONE BMO iteration (one context window).**

BMO spawns a fresh Amp/Claude/Codex instance per iteration with no memory of previous work. If a story is too big, the LLM runs out of context before finishing and produces broken code.
...
Each criterion must be something BMO can CHECK, not something vague.
...
Frontend stories are NOT complete until visually verified. BMO will use the dev-browser skill to navigate to the page, interact with the UI, and confirm changes work.
...
5. **branchName**: Derive from feature name, kebab-case, prefixed with `bmo/`
